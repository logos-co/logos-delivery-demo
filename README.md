...
- A **method-call playground** at the bottom: one card per public `delivery_module` API call, rendered as `methodName(arg…)` with a `Call` button — every interaction is reflected as a row in the event log above. `createNode` spans the full width on top; below it the calls are grouped side by side into **Messaging** (`subscribe`, `unsubscribe`, `send`) and **Channels** (`channelCreate`, `channelExists`, `channelSend`, `channelClose`). `createNode`'s two arguments are fixed-choice enums picked from dropdowns; message payloads are entered as plain **text** (UTF-8-encoded to bytes — the module base64-encodes them into the FFI envelope), while received payloads are arbitrary bytes and are shown as hex, plus a decoded-text row when they are valid UTF-8
