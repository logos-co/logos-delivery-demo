...
- Reading the delivery module's version from Logos Core's module registry (`core_service.getModuleInfo("delivery_module")`) — shown when the host exposes `core_service` (e.g. the `logoscore` daemon)
